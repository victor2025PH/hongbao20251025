...
import { getPublicGroupDetail } from '@/api/publicGroups'
...
export default function GroupDetailPage() {
  ...
  const groupQuery = useQuery<PublicGroupSummary>({
    ...
    queryFn: async () => {
      ...
      return getPublicGroupDetail(groupId)
    },
  ...
  if (groupQuery.isError || (!groupQuery.isLoading && !group)) {
    return (
      <div className="container mx-auto space-y-8 p-6 lg:p-8">
        ...
        <ErrorNotice 
          message={groupQuery.error ? `無法載入群組資料: ${groupQuery.error instanceof Error ? groupQuery.error.message : '未知錯誤'}` : '找不到該群組'} 

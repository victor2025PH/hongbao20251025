...
### 3.7 AI 活动草稿助手
- 需先在环境变量设置 `OPENAI_API_KEY`（可选 `OPENAI_MODEL`，默认 `gpt-4o-mini`）。
- 点击「AI 生成活动」后，会弹出助手对话框，可输入活动需求、目标人群、预算与时长。
- 右侧同时显示最近 10 笔 AI 草稿，可一键套用；草稿会在表单中高亮更新，避免漏看。
- 所有草稿/重用记录都会写入 `public_group_activity_ai_history`，可追踪 prompt、回应、操作者与采用时间。

...
def test_public_group_ai_history_endpoints(monkeypatch: pytest.MonkeyPatch) -> None:
    fake_payload = {
        "name": "AI Auto Draft",
        ...
        "front_card": {
            "title": "AI Highlight",
            "subtitle": "Join within 2 days",
            "cta_label": "Join AI",
            "cta_link": "https://t.me/+ops_lab",
            "badge": "AI 推薦",
            "priority": 60,
        },
    }
    ...
    monkeypatch.setattr(
        "services.ai_activity_generator._call_openai",
        lambda brief: json.dumps(fake_payload),
    )

    token = _extract_csrf("/admin/public-groups/activities")
    draft_resp = client.post(
        "/admin/public-groups/activities/ai-draft",
        json={"brief": "AI draft test"},
        headers={"x-csrf-token": token},
    )
    assert draft_resp.status_code == 200
    draft_body = draft_resp.json()
    assert draft_body["draft"]["name"] == "AI Auto Draft"

    history_resp = client.get("/admin/public-groups/activities/ai-history?limit=5")
    assert history_resp.status_code == 200
    history_items = history_resp.json().get("items") or []
    assert history_items
    history_id = history_items[0]["id"]

    reapply = client.post(
        f"/admin/public-groups/activities/ai-history/{history_id}/reapply",
        json={},
        headers={"x-csrf-token": token},
    )
    assert reapply.status_code == 200
    reapply_body = reapply.json()
    assert reapply_body["draft"]["reward_points"] == 11
